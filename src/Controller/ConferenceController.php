...
class ConferenceController extends AbstractController
{
    ...
    {
        $response = new Response($this->twig - $this->render('conference/index.html.twig', ["conferences" => $conferenceRepository->findAll()]));
    ...
    public function show(Request $request, Conference $conference, CommentRepository $commentRepository, string $photoDir)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($photo = $form['photoFilename']->getData()) {
                $filename = bin2hex(random_bytes(6) . '_' . $photo->guessExtension());
                try {
